...
void particles::store(double x, double y, double z,
...
void particles::retrieve(double *x, double *y, double *z,
                         double *sx, double *sy, double *sz,
    ...
    *p = _weight[_read];
    ...
    if (_read >= _size) _read = 0; // Start over if entire dataset is read
}
